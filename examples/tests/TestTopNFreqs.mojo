...
struct TestTopNFreqs(Movable, Copyable):
    ...
    def __init__(out self, world: World):
        self.world = world
        var p = TopNFreqs(world[].sample_rate, 1024, num_peaks=3, sort_by_freq=True, thresh=-30.0)
    ...
    def next(mut self) -> MFloat[2]:
        ...
        var s = 0.0
        var mul = linlin(self.world[].mouse_x(), 0., 1., 1., 1.2)
        for i in range(len(self.sines)):
             s += self.sines[i].next(self.freqs[i] * mul)
        _ = self.analyzer.next(s)

        var out_pairs = self.analyzer.get_process().get_features_ptr()  # Get a pointer to the freq, amp pairs

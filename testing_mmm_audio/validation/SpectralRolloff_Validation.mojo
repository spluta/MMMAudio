...
struct Analyzer(BufferedProcessable):
    var world: World
    var fft: RealFFT[]
    var rolloffs: List[Float64]
    var sample_rate: Float64

    fn __init__(out self, world: World, sample_rate: Float64):
        self.world = world
        self.fft = RealFFT[](windowsize)
        self.rolloffs = List[Float64]()
        self.sample_rate = sample_rate

    fn next_window(mut self, mut buffer: List[Float64]):
        self.fft.fft(buffer)
        val = SpectralRolloff.from_mags(self.fft.mags, self.sample_rate)
        self.rolloffs.append(val)
        return
...
fn main():
    ...
    pth = "testing_mmm_audio/validation/mojo_results/spectral_rolloff_mojo_results.csv"
    try:
        with open(pth, "w") as f:
            f.write("windowsize,",windowsize,"\n")
            f.write("hopsize,",hopsize,"\n")
            f.write("Rolloff\n")
            for i in range(len(analyzer.process.rolloffs)):
                f.write(String(analyzer.process.rolloffs[i]) + "\n")

...
def parse_args():
    parser = argparse.ArgumentParser(description="Validate spectral spread output.")
    parser.add_argument(
        "--show-plots",
        action="store_true",
...
os.system("mojo run -I . ./testing_mmm_audio/validation/SpectralSpread_Validation.mojo")
print("mojo analysis complete")

with open("./testing_mmm_audio/validation/mojo_results/spectral_spread_mojo_results.csv", "r") as f:
    lines = f.readlines()
...
plt.savefig("testing_mmm_audio/validation/validation_results/spectral_spread_comparison.png")
if show_plots:
    plt.show()
else:
    plt.close()

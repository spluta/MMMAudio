"""Spectral Crest Unit Test

This script tests the Spectral Crest implementation in the MMMAudio library
...
def parse_args():
    parser = argparse.ArgumentParser(description="Validate spectral crest output.")
    parser.add_argument(
        "--show-plots",
        action="store_true",
...
os.system("mojo run -I . ./testing_mmm_audio/validation/SpectralCrest_Validation.mojo")
print("mojo analysis complete")

with open("./testing_mmm_audio/validation/mojo_results/spectral_crest_mojo_results.csv", "r") as f:
    lines = f.readlines()
...
flucoma_crest = flucoma_results["crest"]

plt.figure(figsize=(12, 6))
plt.plot(mojo_crest, label="MMMAudio Spectral Crest (dB)", alpha=0.7)

if flucoma_crest:
    plt.plot(flucoma_crest, label="FluCoMa Spectral Crest", alpha=0.7)

if flucoma_crest:
    try:
        mean_dev_flucoma, std_dev_flucoma = compare_analyses(mojo_crest, flucoma_crest)
        print(f"MMMAudio vs FluCoMa Spectral Crest: Mean Dev = {mean_dev_flucoma:.4f} dB, Std Dev = {std_dev_flucoma:.4f} dB")
    except Exception as e:
        print("Error comparing FluCoMa results:", e)

plt.legend()
plt.ylabel("dB")
plt.title("Spectral Crest Comparison")
plt.savefig("testing_mmm_audio/validation/validation_results/spectral_crest_comparison.png")
if show_plots:
    plt.show()
else:
    plt.close()

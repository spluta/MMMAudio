...
The `.py` file is the interface between Python and Mojo. Open a `.py` example from the examples folder. 
...
Each `.py` file has a corresponding `.mojo` file with the same name. The Mojo file outlines the audio graph structure and the connections between different synths.

All of the UGens used are defined in mmm_dsp.

Mojo is a compiled language. any change to the audio graph means the entire graph needs to be recompiled. this takes about 4-6 seconds on my computer and will happen the first time you run an MMMAudio constructor, like:
...
> Note that in the example Mojo file the name of the file and the name of the main struct that represents the Audio Graph needs to be the same! For example, when trying to run the "FeedbackDelays" example (as above), the Python script looks for a struct named "FeedbackDelays" in the file `FeedbackDelays.mojo`. If you edit the examples or make your own script, be sure to keep this coordination aligned. (In the future this requirement may change for the better.)

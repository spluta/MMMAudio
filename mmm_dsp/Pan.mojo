    ...
    fn next[N: Int](mut self, sample: Float64, pan: Float64, num_speakers: Int64, width: Float64 = 2.0, orientation: Float64 = 0.5) -> SIMD[DType.float64, N]:
        ...
        for i in range(num_speakers):
            if chan_pos[Int(i)] >= 0.5:
                ...
                chan_amp[Int(i)] = self.world[].osc_buffers.read_none(chan_pos[Int(i)], 0)
...
struct Pan2(Representable, Movable, Copyable):
    ...
    fn next(mut self, samples: SIMD[DType.float64, 2], pan: Float64) -> SIMD[DType.float64, 2]:
        ...
        self.gains[0] = self.world[].osc_buffers.at_phase[osc_type=OscType.sine](self.world, linlin(-pan2,-1.0, 1.0, 0.0, 0.25))
        self.gains[1] = self.world[].osc_buffers.at_phase[osc_type=OscType.sine](self.world, linlin(pan2, -1.0, 1.0, 0.0, 0.25))

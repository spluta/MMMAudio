...
@doc_private
struct FFTStereoProcessor[T: FFTStereoProcessable, window_size: Int = 1024](BufferedStereoProcessable):
    """This is a private struct that the user doesn't *need* to see. This is the
    connective tissue between SpectralProcess (which the user *does* see and uses to
    create spectral processes) and BufferedProcess. To learn how this whole family of structs 
    works to create spectral processes, see the `SpectralProcessable` trait.
    ...
    var mags: List[SIMD[DType.float64,2]]
    var phases: List[SIMD[DType.float64,2]]
    ...
    fn __init__(out self, world_ptr: UnsafePointer[MMMWorld], var process: T):
        ...
        self.mags = List[SIMD[DType.float64,2]](length=(window_size // 2) + 1, fill=SIMD[DType.float64,2](0.0))
        self.phases = List[SIMD[DType.float64,2]](length=(window_size // 2) + 1, fill=SIMD[DType.float64,2](0.0))

    @doc_private
    fn next_window(mut self, mut input: List[SIMD[DType.float64,2]]) -> None:
        self.fft.fft(input)
        self.process.next_frame(self.fft.mags,self.fft.phases)
        self.fft.ifft(input)
...
trait FFTStereoProcessable(Movable,Copyable):
    """Implement this trait in a custom struct to pass to `FFTProcess`
    as a Parameter.
    ...
    * `fn get_messages()`: Because `.next_frame()` only runs every `hop_size`
    ...
    every sample to the `SpectralProcess`'s `.next()` function which:
        ...
        * when `FFTProcessor` receives a window of amplitude samples, it performs an
        ...
        to the speakers (or whatever).
    """
    fn next_frame(mut self, mut magnitudes: List[SIMD[DType.float64,2]], mut phases: List[SIMD[DType.float64,2]]) -> None:...
    fn get_messages(mut self) -> None:...

struct FFTStereoProcess[T: FFTStereoProcessable, window_size: Int = 1024, hop_size: Int = 512, input_window_shape: Optional[Int] = None, output_window_shape: Optional[Int] = None, overlap_output: Bool = True](Movable,Copyable):
    """Create an FFTProcess for audio manipulation in the frequency domain.
    ...
    """
    var world_ptr: UnsafePointer[MMMWorld]
    var buffered_process: BufferedStereoProcess[FFTStereoProcessor[T, window_size], window_size, hop_size, input_window_shape, output_window_shape, overlap_output]
    ...
    fn __init__(out self, world_ptr: UnsafePointer[MMMWorld], var process: T):
        ...
        """
        self.world_ptr = world_ptr
        self.buffered_process = BufferedStereoProcess[FFTStereoProcessor[T, window_size], window_size, hop_size, input_window_shape, output_window_shape, overlap_output](self.world_ptr, process=FFTStereoProcessor[T, window_size](self.world_ptr, process=process^))

    fn next(mut self, input: SIMD[DType.float64,2]) -> SIMD[DType.float64,2]:
        """Processes the next input sample and returns the next output sample.
    ...
    fn next_from_buffer(mut self, ref buffer: Buffer, phase: Float64, start_chan: Int = 0) -> SIMD[DType.float64,2]:
        """Returns the next output sample from the internal buffer without processing a new input sample.

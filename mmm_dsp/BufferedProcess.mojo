...
trait BufferedStereoProcessable(Movable, Copyable):
    """Trait that user structs must implement to be used with a BufferedProcess.
    ...
    - get_messages() -> None: This function is called at the top of each audio block to allow the user to retrieve any messages
      they may have sent to this process. Put your message retrieval code here. (e.g. `self.messenger.update(self.param, "param_name")`)
    """
    fn next_window(mut self, mut buffer: List[SIMD[DType.float64, 2]]) -> None:...
    fn get_messages(mut self) -> None:...
...
struct BufferedStereoProcess[T: BufferedStereoProcessable, window_size: Int = 1024, hop_size: Int = 512, input_window_shape: Optional[Int] = None, output_window_shape: Optional[Int] = None,overlap_output: Bool = True](Movable, Copyable):
    """Buffers input samples and hands them over to be processed in 'windows.'
    ...
    var input_buffer: List[SIMD[DType.float64, 2]]
    var passing_buffer: List[SIMD[DType.float64, 2]]
    var output_buffer: List[SIMD[DType.float64, 2]]
    ...
    fn __init__(out self, world_ptr: UnsafePointer[MMMWorld], var process: T, hop_start: Int = 0):
        ...
        self.input_buffer = List[SIMD[DType.float64, 2]](length=window_size * 2, fill=0.0)
        self.passing_buffer = List[SIMD[DType.float64, 2]](length=window_size, fill=0.0)
        self.output_buffer = List[SIMD[DType.float64, 2]](length=window_size, fill=0.0)
        ...
        if self.hop_counter == 0:
            ...
            if input_window_shape:
                for i in range(window_size):
                    index = floor(phase * buffer.get_num_frames()) + i
                    if index < buffer.get_num_frames() and index >= 0:
                        self.passing_buffer[i] = buffer.read_index[2](start_chan, index) * self.input_attenuation_window[i]
                    else:
                        self.passing_buffer[i] = 0.0
            else:
                for i in range(window_size):

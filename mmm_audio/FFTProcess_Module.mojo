...
struct FFTProcess[T: FFTProcessable, window_size: Int = 1024, hop_size: Int = 512, input_window_shape: Int = WindowType.hann, output_window_shape: Int = WindowType.hann](Movable,Copyable):
    ...
    Parameters:
        ...
        input_window_shape: Int specifying what window shape to use to modify the amplitude of the input samples before the FFT. See .Windows -> WindowType for the options.
        output_window_shape: Int specifying what window shape to use to modify the amplitude of the output samples after the IFFT. See .Windows -> WindowType for the options.
    """
    var world: UnsafePointer[MMMWorld]
